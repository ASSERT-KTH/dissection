...
benchmark = "icse15"
path_patch_clustering = os.path.realpath(sys.argv[1])
...
def get_project_features((bug_path, bug_id)):
    ...
    path_diff = os.path.join(bug_path, "patch.diff")
...
for bug_id in sorted(os.listdir(path_benchmark)):
    bug_path = os.path.join(path_benchmark, bug_id)
